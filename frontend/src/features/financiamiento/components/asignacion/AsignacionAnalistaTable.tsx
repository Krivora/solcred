...
import { ESTATUS_STYLES, SECTOR_LABELS, TAMANO_LABELS, formatMonto, formatFecha } from '@/shared/config/solicitudes.config'
import { SolicitanteCell } from '@/shared/components/common/SolicitanteCell'
import type { SolicitudFinanciamiento } from '@/features/financiamiento/types/financiamiento.types'
...
function EstatusBadge({ estatus }: { estatus: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[10px] font-medium whitespace-nowrap', style.className)}>
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center gap-3 py-12 text-center px-6">
        ...
        {onReintentar && <Button variant="outline" size="sm" onClick={onReintentar} className="border-border/60">Reintentar</Button>}
      </div>
    )
  }
  if (solicitudes.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center px-6">
        <div className="p-4 bg-muted/50 rounded-2xl"><Inbox className="h-7 w-7 text-muted-foreground" /></div>
        <div>
          <p className="text-sm font-medium text-foreground">Sin resultados</p>
          <p className="text-xs text-muted-foreground mt-1">No hay solicitudes para asignar con estos filtros</p>
        </div>
      </div>
    )
  }
      ...
      <Table>
        ...
        <TableBody>
          {solicitudes.map((s) => {
            ...
            const monto = formatMonto(s.montoSolicitado ?? null)
            return (
              <TableRow
                key={s.id}
                  ...
                  {s.analistaAsignado ? (
                    <div className="flex flex-col gap-0.5">
                      <span className="text-xs font-medium text-foreground leading-tight">
                        {s.analistaAsignado.analista.usuario.nombre} {s.analistaAsignado.analista.usuario.apellidoPaterno}
                      </span>
                      <span className="text-[11px] text-muted-foreground">{formatFecha(s.analistaAsignado.fechaAsignacion)}</span>
                    </div>
                  ) : (
