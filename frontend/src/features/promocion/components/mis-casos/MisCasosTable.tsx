...
import { FolderOpen, FileText, MoreHorizontal, ClipboardList, RotateCcw, XCircle, SendHorizonal } from 'lucide-react'
            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-muted/30 hover:bg-muted/30 border-b border-border/60">
                        {['Folio', 'Solicitante', 'Programa', 'Tamaño / Sector', 'Recibida', 'Estatus', '', '', ''].map((h, i) => (
    ...
    return (
        <div className="rounded-xl border border-border/60 bg-card flex flex-col items-center justify-center py-20 gap-4">
            <div className="p-4 bg-primary/5 border border-primary/10 rounded-2xl">
                <ClipboardList className="h-7 w-7 text-primary/50" />
            </div>
            <div className="text-center">
                <p className="text-sm font-medium text-foreground">Sin casos asignados</p>
                <p className="text-xs text-muted-foreground mt-1">No tienes casos asignados actualmente</p>
            </div>
        </div>
    )
}
...
export function MisCasosTable({ solicitudes, meta, cargando, onPaginar }: Props) {
                        ...
                        {solicitudes.map((sol) => {
                            ...
                            return (
                                <TableRow
                                    ...
                                    onClick={() => router.push(`/dashboard/gestor/mis-casos/${sol.id}`)}
                                >
                                    {/* Folio */}
                                    ...
                                    <TableCell className="py-3 text-center" onClick={e => e.stopPropagation()}>
                                        <Button
                                            ...
                                            onClick={() => router.push(`/dashboard/gestor/mis-casos/${sol.id}/expediente`)}
                                        >
                                    ...
                                    <TableCell className="py-3 text-center" onClick={e => e.stopPropagation()}>
                                        <Button
                                            ...
                                            className="h-7 w-7 text-muted-foreground hover:text-primary hover:bg-primary/10"
                                            title="Generar PDF"
                                            onClick={() => router.push(`/dashboard/gestor/mis-casos/${sol.id}/pdf`)}
                                        >
                                            ...
                                            <DropdownMenuContent align="end" className="w-64 p-1.5 shadow-lg border-border/60">
                                                ...
                                                >
                                                    ...
                                                    <div className="flex flex-col gap-0">
                                                        <span className="text-xs font-medium text-foreground leading-tight">Enviar para su Aprobacion</span>
